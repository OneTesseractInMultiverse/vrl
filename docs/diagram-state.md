...
Warnings remain visible in `diagnostics` and `diagnosticsText` on successful state. Existing components continue to render SVG for success and a diagnostic block for failure. This extraction does not introduce a warning panel or warning-suppression policy.
...
`application/diagram-state.js` is a pure projection: it selects state fields and uses core's domain diagnostic formatter. It does not compile, render, perform I/O, or know about component props. `composition/diagram-state.js` wires the core compiler and SVG renderer and exposes the sole public function through the package root. The domain and renderer never depend on the shared diagram package or on frameworks.
