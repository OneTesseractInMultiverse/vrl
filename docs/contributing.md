...
Documentation examples must establish behavior, not merely parse. Tag each VRL fence as a document, explicitly contextualized fragment or deliberate failure, and add reviewed expectations for its model facts, diagnostics and physical geometry. Update the governing language/API rule and migration notes in the same change. Run `npm run check:docs` and `make check`; see the [documentation contract workflow](documentation-contracts.md) for source fingerprints, normative tables and verification scope.
