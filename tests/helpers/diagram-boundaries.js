const ALLOWED = {
  "vrl-diagram/src/index.js": ["./composition/diagram-state.js"],
  ...
  "vrl-svelte/src/VrlDiagram.svelte": ["./index.js"],
...
export function diagramDependencyViolations(file, source) {
  ...
  for (const [statement, specifier] of source.matchAll(/\b(?:import|export)\s+(?:[^;]*?\sfrom\s*)?["']([^"']+)["']/g)) {
    ...
    else if (file === "vrl-diagram/src/application/diagram-state.js" && !/^import\s*\{\s*formatDiagnostic\s*\}\s*from/.test(statement)) violations.push(`${file}: only domain diagnostic formatting may be imported`);
