...
This is an additive AST/diagnostic contract change. Consumers with exact property allowlists or serialized snapshots must allow `sourceMap`, `code`, diagnostic `span`, and related-location `span`. Field/identity columns are intentionally more precise than earlier statement-level locations. Model/layout/JSON field shapes remain unchanged.
