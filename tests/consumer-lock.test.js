...
const template = { lockfileVersion: 3, packages: {
  "": { dependencies: { "@subvertic/core": "file:tarballs/core.tgz", react: "19.3.0" } },
  "node_modules/@subvertic/core": { version: "0.1.0", resolved: "file:tarballs/core.tgz", integrity: "old", dependencies: { obsolete: "1.0.0" }, peerDependencies: { obsolete: ">=1" } },
  "node_modules/react": { version: "19.3.0", resolved: "https://registry.npmjs.org/react/-/react-19.3.0.tgz", integrity: "locked" }
} };
const packed = [{ name: "@subvertic/core", version: "0.2.0", integrity: "fresh", manifest: { engines: { node: ">=20.0.0" } } }];
...
test("fresh tarballs replace stale first-party metadata and remove obsolete dependencies", () => {
  ...
  assert.deepEqual(actual.packages["node_modules/@subvertic/core"], { version: "0.2.0", resolved: "file:tarballs/core.tgz", integrity: "fresh", engines: { node: ">=20.0.0" } });
});
...
for (const entry of [undefined, { link: true, resolved: "../../packages/vrl-core" }]) {
  test(`a ${entry ? "linked" : "missing"} package cannot pass as an installed tarball`, () => {
    assert.throws(() => refreshPackedEntries({ packages: { "node_modules/@subvertic/core": entry } }, packed), { message: "Missing packed consumer lock entry: @subvertic/core" });
  });
}
